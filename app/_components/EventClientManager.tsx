...
import {
    Link as LinkIcon, Check, ExternalLink, Copy, Send, Mail, Plus, BarChart3
} from 'lucide-react';
...
export function EventClientManager({ initialEvents, generateAction }: EventClientManagerProps) {
    ...
    // --- 1. LOGIQUE DE PARTAGE (Nouveauté intégrée) ---
    ...
    // --- 2. LOGIQUE STATS (Sécurisée pour MySQL JSON string) ---
    const chartData = activeEvent.responses ? Object.entries(
        activeEvent.responses.reduce((acc: any, curr: any) => {
            // Sécurité : dates peut être une string JSON ou déjà un array
            let dates = [];
            try {
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-8 pb-10">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                {/* LIENS (Intégration Smart Share) */}
                <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 flex flex-col h-96">
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="font-bold text-slate-800 flex items-center gap-2">
                            <LinkIcon size={18} className="text-indigo-600"/> Liens ({activeEvent.tokens?.length || 0})
                        </h3>
                    </div>
                    ...
                    <button
                        ...
                        disabled={generating}
                        className="w-full py-3 bg-indigo-50 text-indigo-700 font-bold rounded-xl mb-4 text-sm hover:bg-indigo-100 transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
                    >
                        <Plus size={16} /> {generating ? "Génération..." : "Générer 5 liens"}
                    </button>

                    <div className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar">
                        {activeEvent.tokens?.map((token: any, i: number) => {
                                ...
                                <div key={token.id} className={cn("p-3 rounded-xl border transition-all text-xs", isUsed ? "bg-slate-50 border-slate-100 opacity-60" : "bg-white border-slate-200")}>
                                    <div className="flex justify-between items-center mb-2">
                                        ...
                                        {isUsed ? (
                                            ...
                                            <span className="text-[10px] bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded-full font-bold">Libre</span>
                                        )}
                                    </div>
                                    ...
                                    {!isUsed && (
                                        <div className="flex gap-2">
                                            <button
                                                ...
                                                title="Copier"
                                            >
                                                {copiedToken === token.id ? <Check size={12}/> : <Copy size={12}/>}
                                                {copiedToken === token.id ? "Copié" : "Copier"}
                                            </button>
                                            <button onClick={() => handleShare('whatsapp', token.id)} className="p-1.5 rounded-lg bg-green-100 text-green-700 hover:bg-green-200 transition-colors" title="WhatsApp"><Send size={14}/></button>
                                            <button onClick={() => handleShare('email', token.id)} className="p-1.5 rounded-lg bg-indigo-100 text-indigo-700 hover:bg-indigo-200 transition-colors" title="Email"><Mail size={14}/></button>
                                        </div>
                                    )}
                                </div>
                            );
            ...
            {/* TABLEAU RÉPONSES (Votre version restaurée avec parsing sécurisé) */}
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                        ...
                        {activeEvent.responses && activeEvent.responses.length > 0 ? activeEvent.responses.map((resp: any) => {
                            // Parsing sécurisé pour l'affichage
                            let dates = [];
